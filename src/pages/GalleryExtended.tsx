...
import { useGalleryData, useGalleryCategories } from "@/hooks/useWebsiteData";
...
const GalleryExtended = () => {
  ...
  // Convert Firestore data to GalleryImage format with size mapping
  const galleryImages: GalleryImage[] = galleryItems.map((item, index) => {
    // Map sizes based on index for variety (you can customize this logic)
    const sizes: ("small" | "medium" | "large" | "wide" | "tall")[] = ["medium", "large", "small", "wide", "tall", "medium"];
  ...
  const availableCategories = categories || ["All", ...Array.from(new Set(galleryImages.map(img => img.category)))];
